        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? 'NBA名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.nbaName" placeholder="NBA名称" clearable></el-input>
                    </el-form-item>
                                                             
                    <el-form-item :label="contents.inputTitle == 1 ? 'NBA类型' : ''">
                        <el-select v-model="searchForm.nbaTypes" placeholder="请选择NBA类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                                    v-for="(item,index) in nbaTypesSelectSearch"
                                    v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('nbaCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/guangyingshipin/upload/nbaCollectionMuBan.xls"
                        >批量导入NBA收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('nbaCollection','导入导出')"
                                ...
                                :on-success="nbaCollectionUploadSuccess"
                                :on-error="nbaCollectionUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('nbaCollection','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入NBA收藏数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('nbaCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "nbaCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="nbaPhoto"
                               header-align="center"
                               width="200"
                               label="NBA照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.nbaPhoto">
                                <img :src="scope.row.nbaPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="nbaVideo"
                                     header-align="center"
                                     width="200"
                                     label="NBA视频">
                        <template slot-scope="scope">
                            <div v-if="scope.row.nbaVideo">
                                <video :src="scope.row.nbaVideo" width="100" height="100" controls="controls">
                                </video>
                            </div>
                            <div v-else>无视频</div>
                        </template>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('nbaCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                            <el-button v-if="isAuth('nbaCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'修改':'' }}</el-button>
                            <el-button v-if="isAuth('nbaCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'删除':'' }}</el-button>
    ...
    //当前表下拉框搜索条件
        ...
        methods: {
            ...
            getDataList() {
                ...
                if (this.searchForm.nbaName!= '' && this.searchForm.nbaName!= undefined) {
                    params['nbaName'] = '%' + this.searchForm.nbaName + '%'
                }
                                                             
                if (this.searchForm.nbaTypes!= '' && this.searchForm.nbaTypes!= undefined) {
                    params['nbaTypes'] = this.searchForm.nbaTypes
                }
                ...
                params['nbaCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "nbaCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //查询级联表搜索条件所有列表
                this.$http({
                    url: "dictionary/page?dicCode=nba_types&page=1&limit=100",
                    method: "get",
                    page: 1,
                    limit: 100,
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.nbaTypesSelectSearch = data.data.list;
                    }
                });
                //查询当前表搜索条件所有列表
            },
            ...
            // 导入功能上传文件成功后调用导入方法
            nbaCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "nbaCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
