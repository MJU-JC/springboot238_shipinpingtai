        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '体育名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.tiyuName" placeholder="体育名称" clearable></el-input>
                    </el-form-item>
                                                             
                    <el-form-item :label="contents.inputTitle == 1 ? '体育类型' : ''">
                        <el-select v-model="searchForm.tiyuTypes" placeholder="请选择体育类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                                    v-for="(item,index) in tiyuTypesSelectSearch"
                                    v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('tiyuCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/guangyingshipin/upload/tiyuCollectionMuBan.xls"
                        >批量导入体育收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('tiyuCollection','导入导出')"
                                ...
                                :on-success="tiyuCollectionUploadSuccess"
                                :on-error="tiyuCollectionUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('tiyuCollection','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入体育收藏数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('tiyuCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "tiyuCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="tiyuPhoto"
                               header-align="center"
                               width="200"
                               label="体育照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.tiyuPhoto">
                                <img :src="scope.row.tiyuPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="tiyuVideo"
                                     header-align="center"
                                     width="200"
                                     label="体育视频">
                        <template slot-scope="scope">
                            <div v-if="scope.row.tiyuVideo">
                                <video :src="scope.row.tiyuVideo" width="100" height="100" controls="controls">
                                </video>
                            </div>
                            <div v-else>无视频</div>
                        </template>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('tiyuCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                            <el-button v-if="isAuth('tiyuCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'修改':'' }}</el-button>
                            <el-button v-if="isAuth('tiyuCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'删除':'' }}</el-button>
    ...
    //当前表下拉框搜索条件
        ...
        methods: {
            ...
            getDataList() {
                ...
                if (this.searchForm.tiyuName!= '' && this.searchForm.tiyuName!= undefined) {
                    params['tiyuName'] = '%' + this.searchForm.tiyuName + '%'
                }
                                                             
                if (this.searchForm.tiyuTypes!= '' && this.searchForm.tiyuTypes!= undefined) {
                    params['tiyuTypes'] = this.searchForm.tiyuTypes
                }
                ...
                params['tiyuCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "tiyuCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //查询级联表搜索条件所有列表
                this.$http({
                    url: "dictionary/page?dicCode=tiyu_types&page=1&limit=100",
                    method: "get",
                    page: 1,
                    limit: 100,
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.tiyuTypesSelectSearch = data.data.list;
                    }
                });
                //查询当前表搜索条件所有列表
            },
            ...
            // 导入功能上传文件成功后调用导入方法
            tiyuCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "tiyuCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            tiyuCollectionUploadError(data){
